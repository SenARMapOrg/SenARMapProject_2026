...
- **`path_edges`**: 経路の各区間（隣接ノード間）。各要素は次の形式です。
  ```json
  {
    ...
    "name": "廊下", "name_display": "",
    "right": "101A", "left": "102",
    "right_display": "ゼミ101A", "left_display": "102教室",
    "length": 5.0,
    "type": "1",
  ...
  - `right` / `left`: この区間の `from`→`to` の進行方向に対して右側・左側にある教室名（`;` 区切り、`edge.csv` にまだ無い建物では空文字）。`edge.csv` には `from`→`to` 片方向の値しか記録されていないが、経路が `edge.csv` の `from/to` と逆向き（`to`→`from`）に通る場合はサーバー側（`build_graph`）で `right`/`left` を入れ替えてから返すため、`path_edges` の `right`/`left` は常にその区間の実際の進行方向を基準にした値になる
  - `name_display` / `right_display` / `left_display`: それぞれ `name`/`right`/`left` の先頭要素（`;`区切りの最初）を、読み上げ・表示向けに解決した表示名。`_first_display_label()` が `data/name.csv`（`_display_name()`と同じ解決順：建物指定 → 全建物共通 → `"{生の名前}教室"`）を使って変換する。ただし `M_Toilet`/`F_Toilet`/`C_Toilet` は `name.csv` に無いため特別扱いし、それぞれ「男子トイレ」「女子トイレ」「多目的トイレ」を返す。該当が無い（元の値が空文字）場合は空文字。navi側の音声案内はこれらの`*_display`フィールドを読み上げに使う（生の`name`/`right`/`left`はトイレ種別コード等が混ざるため読み上げには使わない）
...
- **`dest_side` / `dest_position` / `dest_count` / `dest_display` / `dest_nearest_display`**: 目的地そのもの（検索時に指定した教室名・トイレ種別・食堂名）を、最終区間（`path_edges` の最後の要素、実際に歩く向きに補正済み）の `right`/`left` と厳密照合して求めた位置情報です。`/api/navigate_to_room`（`room`）・`/api/route`（`to_room` 指定時のみ）・`/api/nearest_toilet`（種別コード）・`/api/nearest_cafeteria`（食堂名）で計算されます（`_dest_info()` / `_apply_dest_info()`）。
  - `dest_side`: `"right"`/`"left"`（どちらにも一致しなければ空文字 `""`）
  - `dest_position`: その側の並び（`right`/`left` は`;`区切りで手前から奥への物理的な順序を持つ列）の中で、目的地が手前から数えて何番目かを1始まりで返す（例: `right="10214;10213"` で目的地が`10213`なら`2`）。一致しなければ`null`
  - `dest_count`: その側にある教室の総数。一致しなければ`null`
  - `dest_display`: 目的地そのものの表示名（`name.csv`・トイレ種別による解決済み）。一致しなければ`null`
  - `dest_nearest_display`: その側で一番手前（先頭）にある教室の表示名。一致しなければ`null`
  - navi側は`dest_side`が取れているとき「`<dest_display>`は`<右手/左手>`、`<dest_nearest_display>`から数えて`<dest_position>`番目です」（目的地が一番手前、またはその側に他に教室が無い場合は「`<dest_display>`は`<右手/左手>`です」）を目的地バッジ・到着時の音声案内に使う。`dest_side`が空文字の場合は最終区間の`right_display`/`left_display`を使った簡易フォールバック、それも無理なら汎用文言「この通路沿いが目的地周辺です」を使う
  - 空文字/`null`になるのは次の場合: 目的地エッジに複数の部屋が面していて、右にも左にも何かがある（＝どちらが指定した目的地か区別できない）場合／`to_node`・`to_event`（ノードID・イベント指定）などそもそも「教室名」に相当する値が無い場合／`/api/shortest_path`（ノードID同士の経路探索でこれらのフィールド自体が無い）
