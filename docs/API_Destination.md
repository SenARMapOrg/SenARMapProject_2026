...
- **`dest_side`**: 目的地そのもの（検索時に指定した教室名・トイレ種別・食堂名）が、最終区間（`path_edges` の最後の要素、実際に歩く向きに補正済み）の `right`/`left` のどちらに厳密一致したかを `"right"`/`"left"` で返します。`/api/navigate_to_room`（`room`）・`/api/route`（`to_room` 指定時のみ）・`/api/nearest_toilet`（種別コード）・`/api/nearest_cafeteria`（食堂名）で計算されます（`_dest_side()`）。以下の場合は空文字 `""` になり、この場合navi側は目的地バッジを汎用文言（「この通路沿いが目的地周辺です」）にフォールバックします:
  - 目的地エッジに複数の部屋が面していて、右にも左にも何かがある（＝どちらが指定した目的地か区別できない）
  - `to_node`・`to_event`（ノードID・イベント指定）など、そもそも「教室名」に相当する値が無い
  - `/api/shortest_path`（ノードID同士の経路探索で `dest_side` フィールド自体が無い）
