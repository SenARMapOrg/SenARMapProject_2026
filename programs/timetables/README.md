...
登録日・最終ログイン** と、**管理画面の閲覧記録** だけで、時間割の中身（科目名・教室）は出さない
（公開範囲を「非公開」にした利用者の期待を裏切らないため）。閲覧専用で、削除などの操作はできない。

管理者はトップページから普段どおりログインしてから `/admin` を開く。**管理者以外（ログインしていない人を
含む）が `/admin` を開くと、ページを返さずにトップページへリダイレクトする**。

**初回の設定（この順番で行う）**

1. **マイグレーションを本番に適用する**: `npm run db:migrate:remote`（`0008_add_admin_audit_log.sql` で
   閲覧記録のテーブルを作る）。適用前は、管理者でも一覧は表示されない（下の「閲覧記録」参照）
2. main にマージしてデプロイする
3. Cloudflare ダッシュボード → このPagesプロジェクト → Settings → Variables and Secrets で、
   **Production** に `ADMIN_EMAILS` を **Secret**（暗号化）として追加し、再デプロイする。
   値は管理者の大学メールアドレスをカンマ区切りで並べたもの（例: `a@senshu-u.jp,b@senshu-u.jp`）
...
**閲覧記録**（`admin_audit_log` テーブル、1年保存）

次の3種類を、日時・ユーザーID・メールアドレス・パス・接続元IP・User-Agent と一緒に D1 に保存する。
直近100件は管理画面の下部で見られる。1年より古い記録は、新しい記録を書くたびに自動で削除する。

| 種類 | 記録するタイミング |
|---|---|
| `admin_access`（閲覧） | 管理者が一覧・閲覧記録を取得したとき |
| `admin_denied`（拒否・管理API） | 管理者でないログイン済みユーザーが `/api/admin/*` を叩いたとき |
| `admin_page_denied`（拒否・管理画面） | 管理者でないログイン済みユーザーが `/admin` を開いたとき |

ログインしていない人のアクセスは記録しない（自動巡回のボットで記録が埋まるのを避けるため）。
**閲覧記録を書けなかった場合は一覧を表示しない**（503）。記録が欠けた閲覧を作らないため。
拒否の記録は書けなくても、締め出す動作そのものは変わらない。

**セキュリティ上の仕組み**（`functions/admin.ts`・`functions/api/_lib/admin*.ts`・`routes/admin.ts`）:

- 管理者かどうかは **サーバー側でだけ** 判定する。ページ（`/admin`）もAPI（`/api/admin/*`）も、
  それぞれ独立に判定する。画面の出し分けは見た目だけ
- 管理者以外には、ページはトップへのリダイレクト、APIは **404** を返し、中身を一切返さない
- **管理画面のHTMLは静的ファイルとして置かない**。`npm run build` の最後に `scripts/embed-admin-page.mjs` が
  `dist/admin.html` を Function（`functions/api/_lib/admin-page.generated.ts`、Git管理外）に埋め込んで
  静的ファイルから消し、管理者と確認できたときだけ `functions/admin.ts` が返す。
  静的ファイルのままだと、Pages の静的配信はURLを正規化してからファイルを探す（`//admin` や `/%61dmin` でも
  `admin.html` を返す）一方、Functions の振り分けは正規化前のパスで判定するため、門番を迂回できてしまう
  （2026-09-25 の点検で本番で確認し、この方式に変更した）
- 管理APIは、自サイトのページからの `fetch`（`Sec-Fetch-Site: same-origin`）だけを受け付ける。
  別サイトのリンクや画像などを使って、管理者のブラウザに管理APIを叩かせる攻撃を止める（Fetch Metadata）
...
- ページとAPIの両方に、CSP（自サイト以外のスクリプト・通信を禁止）、埋め込み禁止
  ...
  リファラ送信禁止、別サイトからの参照禁止（COOP/CORP）のヘッダを付ける
  （Functions の応答には `public/_headers` が効かないので `ADMIN_SECURITY_HEADERS` としてコードで付ける）
- 利用者向けの画面にも `public/_headers` で CSP と埋め込み禁止を付けている。管理APIは同じサイトからの
  リクエストを通すので、利用者向けの画面で XSS が起きると管理者のブラウザ経由で一覧を取られうる。
  画面側は利用者の入力を必ず `textContent` かエスケープして表示し、CSP で二重に防ぐ
...
- **プレビュー環境を本番DBから切り離す（2026-09-25 の点検で判明・最優先）**: Pages プロジェクトの
  プレビュー環境の D1 バインディングが本番と同じ `timetables-db` を指しており、しかも全ブランチが
